...
struct PerformanceTuning {
    ...
    static PerformanceTuning ForMemory() {
        return PerformanceTuning{.enabled = true,
                                 .journal_mode = "MEMORY",
                                 .synchronous = "OFF",
                                 .page_size = 4096,
                                 .cache_size_kb = 8192,
    ...
    static PerformanceTuning Disabled() { return PerformanceTuning{.enabled = false}; }
};
...
struct DatabaseConfig {
    ...
    static DatabaseConfig Memory(PerformanceTuning tuning = PerformanceTuning::ForMemory()) {
        return DatabaseConfig{.mode = DatabaseMode::Memory, .tuning = tuning};
    }

    static DatabaseConfig NativeFile(const std::string& filepath,
                                     PerformanceTuning tuning = PerformanceTuning::ForNativeFile()) {
        return DatabaseConfig{.mode = DatabaseMode::NativeFile, .path = filepath, .tuning = tuning};
    }

    static DatabaseConfig OPFS(const std::string& filepath, PerformanceTuning tuning = PerformanceTuning::ForOPFS()) {
        return DatabaseConfig{.mode = DatabaseMode::OPFS, .path = filepath, .tuning = tuning};
    }
};
